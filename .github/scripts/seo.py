...
SITEMAP_BLOG = 'https://blog.atendeaqui.app/sitemap.xml'
BASE = 'https://www.atendeaqui.app'
...
PROIBIDAS = [
    ...
    ('prova social inventada',
     ...
     r'(empresas|clientes|usu[áa]rios|atendentes|conversas)\b'),
    ('métrica de acurácia ou disponibilidade',
     r'\d{2}[,.]?\d*\s*%\s*(de\s*)?(acerto|precis[ãa]o|uptime|disponibilidade)|99[,.]9'),
    # ⚠️ `\b` obrigatório — nome curto de concorrente casa dentro de palavra comum.
    # Mesma classe de bug do `ucondo` dentro de "seucondomínio", achado no motor do blog.
    ('concorrente pelo nome',
     r'\b(zenvia|take\s*blip|blip\.ai|huggy|octadesk|zendesk|intercom|sleekflow|digisac|chatpro|chatwoot)\b'),
    # O WhatsApp roda via Z-API. Dizer "API oficial" é declaração falsa sobre a
